...
-- Now, to delete  them in order to  clean the dataset
DELETE FROM hotel_bookings
WHERE adults = 0 AND children = 0 AND babies = 0;

-- If the dates are stored as 'YYYY-MM-DD' text, convert them:
UPDATE hotel_bookings
SET reservation_status_date = STR_TO_DATE(reservation_status_date, '%Y-%m-%d');

-- Modify the column type to officially be a DATE (if it was imported as VARCHAR/Text)
